...
可以看到,这两个操作,都是到函数对应的Closure指针中的env表中去查询数据.因此,如果执行以下的Lua代码,实际上会查找不到Lua标准库提供的print函数,提示报错"attempt to call global 'print' (a nil value)",原因就是首先使用setfenv函数将当前函数的env表置为了一个空表,此时在当前函数的env表中查找不到这个名字的函数了:
